                ...
                List(model.items.lazy.filter(model.searchState()), selection: $model.selected) { item in
                    ...
                    HistoryRowView(
                        model: .init(
                            ...
                            excludeApp: { [weak model] in
                                if let bundleId = item.bundleId {
                                    model?.excludeApp(bundleId: bundleId)
                                }
                            },
                            removeExclusion: { [weak model] in
                                if let app = item.bundleId {
                                    model?.removeExcludeApp(app: app)
                                }
                            },
                            isAppExcluded: model.excludeApps.contains(where: { item.bundleId == $0 })
                        )
                    )
//                    .contentShape(Rectangle())
                ...
                .onDeleteCommand { [weak model] in
                    guard let model, let selected = model.selected else { return }
                    model.remove(item: selected)
                }
            }
        }
        .onAppear {
